...
try:
    ...
    raise ImportError(
        "The 'datasets', 'huggingface_hub' and 'plotly' packages are required. "
        "Please install them with: pip install datasets huggingface_hub plotly"
    ) from e
...
class SPORCDataset:
    ...
    def __init__(self, cache_dir: Optional[str] = None, use_auth_token: Optional[str] = None,
                 ...
                 load_turns_eagerly: bool = True, load_samples_only: bool = False):
        """
        ...
        Args:
            ...
            load_samples_only: If True, only load the sample dataset files
                       (episodeLevelDataSample.jsonl.gz and speakerTurnDataSample.jsonl.gz)
                       and skip the full dataset files.
        """ 
        ...
        self.load_samples_only = load_samples_only
    ...
    def _validate_local_files(self) -> Dict[str, str]:
        ...
        If `load_samples_only=True`, only the sample files are required.
        ...
        if self.load_samples_only:
            logger.info("Loading SPORC sample dataset only.")
            expected_files = {
                'episode_data_sample': self.LOCAL_FILES['episode_data_sample'],
                'speaker_turn_data_sample': self.LOCAL_FILES['speaker_turn_data_sample']
            }
        else:
            expected_files = self.LOCAL_FILES
        ...
        if missing_files:
            raise DatasetAccessError(
                ...
                f"Expected files: {', '.join(expected_files.values())}"
            )
    ...
    def _process_data(self) -> None:
        ...
        # Determine expected record count based on dataset size
        if self.load_samples_only:
            expected_total = 210_000   # size of sample subset
        else:
            expected_total = 1_134_058  # full dataset size

        tqdm_bar = tqdm(self._dataset, total=expected_total, disable=not self.show_progress, desc="Loading episodes")
    ...
    def plot_category_sunburst(self, min_count: int = 5, palette: str = "Pastel"):
        """
        Plot a normalized category hierarchy sunburst from a SPORC dataset.

        Args:
            dataset: SPORCDataset instance or any object with .get_dataset_statistics()
            min_count: Minimum number of episodes per category to include
            palette: Plotly qualitative color palette name (e.g., 'Pastel', 'Safe', 'Set3')
        ...
        def _norm(name: str) -> str:
            """Normalise category name for case-insensitive lookups."""
        ...
        cat_df = (
            pd.DataFrame(list(cat_dist.items()), columns=["category", "count"])
            .query("category != '' and count > @min_count")
        )

        # Normalise names
        cat_df["category"] = cat_df["category"].apply(_norm)

        # Assign main categories
        def assign_main_category(cat: str) -> str:
            return cat if is_main_category(cat) else get_main_category(cat)

        cat_df["main_category"] = cat_df["category"].apply(assign_main_category)
        cat_df["main_category"] = cat_df["main_category"].apply(_norm)

        # Combine duplicates
        cat_df = cat_df.groupby(["main_category", "category"], as_index=False)["count"].sum()

        # Pick color palette
        color_sequence = getattr(px.colors.qualitative, palette, px.colors.qualitative.Pastel)
        ...
        fig = px.sunburst(
            ...
            values="count",
            color="main_category",
            title="SPORC Dataset: Category Hierarchy & Distribution",
            height=800,
            width=800,
        ...
        fig.update_layout(
            title={
                'text': "SPORC Dataset: Category Hierarchy & Distribution",
                'x': 0.5,
                'xanchor': 'center',
                'yanchor': 'top',
